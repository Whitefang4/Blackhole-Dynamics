...
### Define initial condition, time domains
...
plt = plot(tsteps, waveform,
           ...
           ### Decide the neural network architecture to be used for the UDE
...
callback4(θ,l,pred_waveform; doplot = true) = begin
    push!(losses, l)
    display(l)
...
res1 = Optimization.solve(optprob, OptimizationOptimisers.Adam(0.001f0), callback=callback4, maxiters=100)
optprob = Optimization.OptimizationProblem(optf, res1.u)
res2 = Optimization.solve(optprob, BFGS(initial_stepnorm=0.01, linesearch=LineSearches.BackTracking()), callback=callback4, maxiters=20)
